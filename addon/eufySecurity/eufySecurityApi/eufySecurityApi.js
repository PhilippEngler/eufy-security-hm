...
class EufySecurityApi {
    ...
    retries = 0;
    serviceState = "init";
    captchaState = { captchaId: "", captcha: "" };
    tfaCodeRequested = false;
    taskUpdateState;
    waitUpdateState;
    ...
    constructor() {
        ...
        (0, logging_1.setLoggingLevel)("http", this.config.getLogLevelHttp());
    ...
    async initialize() {
        if (this.config.getEmailAddress() === "" || this.config.getPassword() === "") {
            ...
            if (this.config.getClientPrivateKey() === undefined || this.config.getClientPrivateKey() === "" || this.config.getServerPublicKey() === undefined || this.config.getServerPublicKey() === "") {
                logging_1.rootAddonLogger.debug("Incomplete persistent data for v2 encrypted cloud api communication. Invalidate authenticated session data.");
            ...
            this.httpService = await http_1.HTTPApi.initialize(this.config.getCountry(), this.config.getEmailAddress(), this.config.getPassword(), this.httpApiPersistentData);
            ...
            if (this.config.getOpenudid() === "") {
                ...
                logging_1.rootAddonLogger.debug("Generated new openudid", { openudid: this.config.getOpenudid() });
            }
            ...
            if (this.config.getSerialNumber() === "") {
                ...
                logging_1.rootAddonLogger.debug("Generated new serial_number", { serialnumber: this.config.getSerialNumber() });
            }
            this.httpService.setSerialNumber(this.config.getSerialNumber());
            if (this.config.getPushServiceActive() === true) {
                logging_1.rootAddonLogger.info("Started initializing push notification connection.");
                try {
     ...
     */
    closePushService() {
        logging_1.rootAddonLogger.info("Stopping PushService...");
        if (this.pushService) {
     ...
     */
    closeMqttService() {
        logging_1.rootAddonLogger.info("Stopping MqttService...");
        if (this.mqttService) {
     ...
     */
    async closeStation() {
        logging_1.rootAddonLogger.info("Closing connections to all stations...");
    ...
    async connect(options) {
        await this.httpService.login(options)
            ...
            logging_1.rootAddonLogger.error("Connect Error", { error: (0, utils_1.getError)(error), options: options });
        });
    }
    /**
    ...
    async onAPIClose() {
        ...
        if (this.retries < 3) {
            ...
            logging_1.rootAddonLogger.error(`Tried to re-authenticate to Eufy cloud, but failed in the process. Manual intervention is required!`);
        }
    }
    /**
    ...
    async onAPIConnect() {
        ...
        if (loginData) {
            ...
            logging_1.rootAddonLogger.warn("No mqtt login data received. Skipping creating mqtt service.");
        }
        ...
        this.stations.on("station removed", (station) => this.onStationRemoved(station));
        this.stations.on("station image download", (station, file, image) => this.onStationImageDownload(station, file, image));
        this.stations.on("station database query local", (station, returnCode, data) => this.onStationDatabaseQueryLocal(station, returnCode, data));
        this.stations.on("station database query latest", (station, returnCode, data) => this.onStationDatabaseQueryLatest(station, returnCode, data));
        ...
        this.devices.on("device removed", (device) => this.onDeviceRemoved(device));
    ...
    onAuthTokenRenewed(token, token_expiration) {
        ...
        logging_1.rootAddonLogger.info(`The authentication token has been renewed.`);
    }
    /**
    ...
    onStationRemoved(station) {
        logging_1.rootAddonLogger.info(`The station ${station.getSerial()} has been removed.`);
    }
    /**
    ...
    async onStationImageDownload(station, file, image) {
     ...
     * @param data The result data.
     */
    async onStationDatabaseQueryLatest(station, returnCode, data) {
        if (returnCode === 0 && data && data.length > 0) {
            for (let i = 0; i < data.length; i++) {
                try {
                    const response = data[i];
                    if (response.device_sn === undefined || !(this.devices.existDevice(response.device_sn))) {
                        logging_1.rootAddonLogger.debug(`StationDatabaseQueryLatest: Station ${station.getSerial()} has no device with serial ${response.device_sn}.`);
                        continue;
                    }
                    if (response.crop_local_path === "") {
                        logging_1.rootAddonLogger.debug(`StationDatabaseQueryLatest: Empty path detected.`, JSON.stringify(response));
                        continue;
                    }
                    const device = await this.devices.getDevice(response.device_sn);
                    if (device.hasProperty(http_1.PropertyName.DevicePictureTime)) {
                        const pictureTimeValue = device.getPropertyValue(http_1.PropertyName.DevicePictureTime);
                        if (pictureTimeValue === 0) {
                            logging_1.rootAddonLogger.debug(`StationDatabaseQueryLatest: Initialize the pictureTime property.`, { station: station.getSerial(), device: device.getSerial(), pictureTimeValue: pictureTimeValue, filePath: response.crop_local_path });
                            const imageTime = (0, utils_3.getDateTimeFromImageFilePath)(response.crop_local_path);
                            if (imageTime === undefined) {
                                this.updateCameraEventTimeSystemVariable(response.device_sn, -1);
                                device.setCustomPropertyValue(http_1.PropertyName.DevicePictureTime, -1);
                            }
                            else {
                                this.updateCameraEventTimeSystemVariable(response.device_sn, imageTime.valueOf());
                                device.setCustomPropertyValue(http_1.PropertyName.DevicePictureTime, imageTime.valueOf());
                            }
                        }
                        else if (pictureTimeValue > 0) {
                            logging_1.rootAddonLogger.debug(`StationDatabaseQueryLatest: The pictureTime property already set.`, { station: station.getSerial(), device: device.getSerial(), pictureTimeValue: pictureTimeValue });
                            this.updateCameraEventTimeSystemVariable(response.device_sn, pictureTimeValue);
                        }
                        else {
                            logging_1.rootAddonLogger.error(`StationDatabaseQueryLatest: There was an previous error retrieving the pictureTime.`, { station: station.getSerial(), device: device.getSerial() });
                        }
                    }
                }
                catch (e) {
                    logging_1.rootAddonLogger.error(`StationDatabaseQueryLatest: Error occured: ${e.message}`);
                    continue;
                }
            }
        }
    ...
    onDeviceRemoved(device) {
        logging_1.rootAddonLogger.info(`The device ${device.getSerial()} has been removed.`);
    }
    /**
     ...
     * @returns The object.
     */
    async getDevicesFromStation(baseSerial) {
     ...
     * @returns The device as object.
     */
    async getStationDevice(stationSerial, channel) {
    ...
    saveCloudToken() {
        ...
        if (!!token && !!token_expiration) {
            logging_1.rootAddonLogger.debug("Save cloud token and token expiration", { token: token, tokenExpiration: token_expiration });
    ...
    async refreshCloudData() {
        if (this.config.getAcceptInvitations()) {
            await this.processInvitations().catch(err => {
                const error = (0, error_1.ensureError)(err);
                logging_1.rootAddonLogger.error("Error in processing invitations", { error: (0, utils_1.getError)(error) });
            });
        }
        await this.httpService.refreshAllData().catch(err => {
            const error = (0, error_1.ensureError)(err);
            logging_1.rootAddonLogger.error("Error during API data refreshing", { error: (0, utils_1.getError)(error) });
        });
        if (this.refreshEufySecurityCloudTimeout !== undefined) {
            clearTimeout(this.refreshEufySecurityCloudTimeout);
        }
    ...
    async refreshData(target) {
        ...
        switch (target) {
            case "all":
                ...
                result.set("houses", await (0, utils_4.waitForHttpApiEvent)(this.httpService, "houses", 1000));
                result.set("stations", await (0, utils_4.waitForHttpApiEvent)(this.httpService, "hubs", 1000));
                break;
            case "devices":
                result.set("devices", await (0, utils_4.waitForHttpApiEvent)(this.httpService, "devices", 1000));
                break;
     ...
     */
    generateNewTrustedDeviceName() {
        return const_1.PhoneModels[(0, utils_3.randomNumber)(0, const_1.PhoneModels.length)];
    }
    /**
     * Generate a new random trusted device name and return a JSON resonse.
     * @returns The trusted device name as JSON string.
     */
    generateNewTrustedDeviceNameJson() {
        let json = {};
    ...
    async getHousesAsJson() {
        ...
        try {
            ...
            logging_1.rootAddonLogger.error(`Error occured at getHousesAsJson. Error: ${e.message}.`, JSON.stringify(e));
            this.setLastConnectionInfo(false);
    ...
    async getHouseAsJson(houseId) {
        ...
        try {
            ...
            logging_1.rootAddonLogger.error(`Error occured at getHouseAsJson. Error: ${e.message}.`, JSON.stringify(e));
    ...
    */
    ...
    async getDevicesAsJson() {
        ...
        try {
            ...
            logging_1.rootAddonLogger.error(`Error occured at getDevicesAsJson. Error: ${e.message}.`, JSON.stringify(e));
            this.setLastConnectionInfo(false);
            json = { "success": false, "reason": e.message };
        }
        return JSON.stringify(json);
     ...
     * @returns The picture.
     */
    async getDeviceImage(deviceSerial) {
        try {
            ...
            logging_1.rootAddonLogger.error(`Error occured at getDeviceImage. Error: ${e.message}.`, JSON.stringify(e));
        }
        return null;
    }
    /**
    ...
    async getDeviceAsJson(deviceSerial) {
        ...
        try {
            ...
            logging_1.rootAddonLogger.error(`Error occured at getDeviceAsJson. Error: ${e.message}.`, JSON.stringify(e));
            this.setLastConnectionInfo(false);
            json = { "success": false, "reason": e.message };
        }
        return JSON.stringify(json);
    ...
    async getDevicePropertiesMetadataAsJson(deviceSerial) {
        ...
        try {
            if (this.devices) {
                const device = await this.devices.getDevice(deviceSerial);
                if (device) {
                    json = { "success": true, "version": this.getEufySecurityApiVersion(), "model": device.getModel(), "modelName": (0, utils_4.getModelName)(device.getModel()), "isDeviceKnownByClient": Object.values(http_1.DeviceType).includes(device.getDeviceType()), "deviceType": (0, utils_4.getDeviceTypeAsString)(device), "data": device.getPropertiesMetadata() };
                    this.setLastConnectionInfo(true);
                }
                else {
                    json = { "success": false, "reason": `The device with serial ${deviceSerial} does not exists.` };
            ...
            logging_1.rootAddonLogger.error(`Error occured at getDevicePropertiesMetadataAsJson. Error: ${e.message}.`, JSON.stringify(e));
            this.setLastConnectionInfo(false);
            json = { "success": false, "reason": e.message };
        }
        return JSON.stringify(json);
    ...
    async getDevicePropertiesAsJson(deviceSerial) {
        ...
        try {
            ...
            logging_1.rootAddonLogger.error(`Error occured at getDevicePropertiesAsJson. Error: ${e.message}.`, JSON.stringify(e));
            this.setLastConnectionInfo(false);
            json = { "success": false, "reason": e.message };
        }
        return JSON.stringify(json);
    ...
    async getDeviceParams(deviceSerial) {
        ...
        try {
            ...
            logging_1.rootAddonLogger.error(`Error occured at getDeviceParams. Error: ${e.message}.`, JSON.stringify(e));
            this.setLastConnectionInfo(false);
            json = { "success": false, "reason": e.message };
        }
        return JSON.stringify(json);
    ...
    async setDeviceProperty(deviceSerial, propertyName, propertyValue) {
        ...
        try {
            if (this.devices) {
                ...
                try {
                    const res = await (0, utils_4.waitForDeviceEvent)(await this.devices.getDevice(deviceSerial), "property changed", 10000, this.devices, propertyName, propertyValue).then(() => {
                        return true;
                    }, (value) => {
                    ...
                    logging_1.rootAddonLogger.error(`Error occured at setDeviceProperty.`, JSON.stringify(e));
            ...
            logging_1.rootAddonLogger.error(`Error occured at setDeviceProperty. Error: ${e.message}.`, JSON.stringify(e));
            this.setLastConnectionInfo(false);
            json = { "success": false, "reason": e.message };
        }
        return JSON.stringify(json);
     ...
     */
    async makeJsonObjectForStation(station) {
        const properties = station.getProperties();
        let json = {};
        let privacyMode = undefined;
    ...
    async getStationsAsJson() {
        ...
        try {
            ...
            logging_1.rootAddonLogger.error(`Error occured at getStationsAsJson. Error: ${e.message}.`, JSON.stringify(e));
            this.setLastConnectionInfo(false);
            json = { "success": false, "reason": e.message };
        }
        return JSON.stringify(json);
    ...
    async getStationPropertiesMetadataAsJson(stationSerial) {
        ...
        try {
            if (this.stations) {
                ...
                if (station) {
                    json = { "success": true, "version": this.getEufySecurityApiVersion(), "type": station.getModel(), "modelName": (0, utils_4.getModelName)(station.getModel()), "isDeviceKnownByClient": Object.values(http_1.DeviceType).includes(station.getDeviceType()), "deviceType": (0, utils_4.getStationTypeString)(station), "data": station.getPropertiesMetadata() };
                    this.setLastConnectionInfo(true);
                }
                else {
            ...
            logging_1.rootAddonLogger.error(`Error occured at getStationPropertiesMetadataAsJson. Error: ${e.message}.`, JSON.stringify(e));
            this.setLastConnectionInfo(false);
            json = { "success": false, "reason": e.message };
        }
        return JSON.stringify(json);
    ...
    async getStationPropertiesAsJson(stationSerial) {
        ...
        try {
            ...
            logging_1.rootAddonLogger.error(`Error occured at getStationPropertiesAsJson. Error: ${e.message}.`, JSON.stringify(e));
            this.setLastConnectionInfo(false);
            json = { "success": false, "reason": e.message };
        }
        return JSON.stringify(json);
    ...
    async getStationParams(stationSerial) {
        ...
        try {
            ...
            logging_1.rootAddonLogger.error(`Error occured at getStationParams. Error: ${e.message}.`, JSON.stringify(e));
            this.setLastConnectionInfo(false);
            json = { "success": false, "reason": e.message };
        }
        return JSON.stringify(json);
    ...
    async setStationProperty(stationSerial, propertyName, propertyValue) {
        ...
        try {
            if (this.stations) {
                ...
                try {
                    const res = await (0, utils_4.waitForStationEvent)(await this.stations.getStation(stationSerial), "property changed", 10000, undefined, this.stations, propertyName, propertyValue).then(() => {
                        return true;
                    }, (value) => {
                        if (typeof value === "boolean") {
                            return false;
            ...
            logging_1.rootAddonLogger.error(`Error occured at setStationProperty. Error: ${e.message}.`, JSON.stringify(e));
            this.setLastConnectionInfo(false);
            json = { "success": false, "reason": e.message };
        }
        return JSON.stringify(json);
     ...
     * @returns A JSON-String.
     */
    async rebootStation(stationSerial) {
        let json = {};
        try {
            if (this.stations) {
                if (!this.stations.existStation(stationSerial)) {
            ...
            logging_1.rootAddonLogger.error(`Error occured at rebootStation. Error: ${e.message}.`, JSON.stringify(e));
            this.setLastConnectionInfo(false);
            json = { "success": false, "reason": e.message };
        }
        return JSON.stringify(json);
    ...
    async getStationAsJson(stationSerial) {
        ...
        try {
            ...
            logging_1.rootAddonLogger.error(`Error occured at getStationAsJson. Error: ${e.message}.`, JSON.stringify(e));
            this.setLastConnectionInfo(false);
            json = { "success": false, "reason": e.message };
        }
        return JSON.stringify(json);
     ...
     */
    async saveStationsSettings(stations) {
        if (this.stations) {
            for (const stationSerial in stations) {
    ...
    async getGuardMode() {
        ...
        try {
            if (this.stations) {
                ...
                if (stations) {
                    ...
                    for (const stationSerial in stations) {
                        json.data.push(await this.makeJsonObjectForStation(stations[stationSerial]));
                        if (mode === -1) {
                            mode = stations[stationSerial].getGuardMode();
                        }
            ...
            logging_1.rootAddonLogger.error(`Error occured at getGuardMode. Error: ${e.message}.`, JSON.stringify(e));
            this.setLastConnectionInfo(false);
            json = { "success": false, "reason": e.message };
        }
        return JSON.stringify(json);
    ...
    async getGuardModeStation(stationSerial) {
        ...
        try {
            if (this.stations) {
                ...
                if (station) {
                    ...
                    if (station.hasProperty(http_1.PropertyName.StationGuardModeTime)) {
                        this.setSystemVariableTime("eufyLastModeChangeTime" + station.getSerial(), new Date(station.getPropertyValue(http_1.PropertyName.StationGuardModeTime)));
                    }
                    else {
            ...
            logging_1.rootAddonLogger.error(`Error occured at getGuardModeStation. Error: ${e.message}.`, JSON.stringify(e));
            this.setLastConnectionInfo(false);
            json = { "success": false, "reason": e.message };
        }
        return JSON.stringify(json);
    ...
    async setGuardMode(guardMode) {
        ...
        try {
                ...
                if (res === true) {
                    ...
                    for (const stationSerial in stations) {
                        ...
                        if (guardMode === station.getGuardMode()) {
                            ...
                            this.updateStationGuardModeSystemVariable(station.getSerial(), station.getGuardMode());
                            logging_1.rootAddonLogger.error(`Error occured at setGuardMode: Failed to switch mode for station ${station.getSerial()}.`);
                        }
                    }
                    if (err === 0) {
            ...
            logging_1.rootAddonLogger.error(`Error occured at setGuardMode: ${e.message}.`);
            this.setLastConnectionInfo(false);
            json = { "success": false, "reason": e.message };
        }
        return JSON.stringify(json);
        ...
        try {
            if (this.stations) {
                if (this.devices.existDevice(stationSerial) === true) {
                    const device = await this.devices.getDevice(stationSerial);
        ...
        if (this.devices.existDevice(deviceSerial) === true) {
            ...
            if (device.isPanAndTiltCamera()) {
                if (device.isEnabled() === value) {
                    ...
                    try {
                        const res = await (0, utils_4.waitForDeviceEvent)(await this.devices.getDevice(deviceSerial), "property changed", 10000, this.devices, http_1.PropertyName.DeviceEnabled, value).then(() => {
                            return true;
                        }, (value) => {
                            if (typeof value === "boolean") {
                                return false;
                        ...
                        return `{"success":${res},"enabled":${res === true ? value : !value}}`;
                    }
                    catch (e) {
        ...
        try {
            ...
            if (target === "all") {
                if (res.get("devices") === true && res.get("houses") === true && res.get("stations") === true) {
                    ...
                    json = { "success": false, "data": (0, utils_4.convertMapToObject)(res) };
                }
            }
            else {
                json = { "success": res.get(target), "data": (0, utils_4.convertMapToObject)(res) };
            }
        }
        catch (e) {
            logging_1.rootAddonLogger.error(`Error occured at refreshCloudDeviceData: ${e.message}.`);
            this.setLastConnectionInfo(false);
            json = { "success": false, "reason": e.message.replaceAll(`"`, `'`) };
        }
        return JSON.stringify(json);
                ...
                if (!station.isP2PConnectableDevice()) {
                    ...
                    if (connected === true) {
                        try {
                            ...
                            logging_1.rootAddonLogger.error(`Error during closing p2p connection to station ${stationSerial}. Got value '${JSON.stringify(e)}'.`);
                    ...
                    if (connected === false) {
                        try {
                            ...
                            logging_1.rootAddonLogger.error(`Error during reconnecting p2p connection to station ${stationSerial}. Got value '${JSON.stringify(e)}'.`);
     ...
     */
    async updateGlobalStationGuardModeChangeTimeSystemVariable() {
        const stations = await this.stations.getStations();
        let station;
        let tempModeChange;
        ...
        for (const stationSerial in stations) {
            station = stations[stationSerial];
            if (station.hasProperty(http_1.PropertyName.StationGuardModeTime)) {
                tempModeChange = new Date(station.getPropertyValue(http_1.PropertyName.StationGuardModeTime));
     ...
     */
    getLocalStaticUdpPortForStation(stationSerial) {
    ...
    async checkSystemVariables() {
        ...
        if (availableSystemVariables === undefined) {
            ...
            try {
                ...
                logging_1.rootAddonLogger.error(`Error occured at checkSystemVariables. Error: ${e.message}.`, JSON.stringify(e));
                this.setLastConnectionInfo(false);
                json = { "success": false, "reason": e.message };
            }
        }
     ...
     * @param command The command to be executed.
     */
    setInteraction(serialNumber, eventInteractionType, target, useHttps, useLocalCertificate, rejectUnauthorized, command, user, password) {
        let json = {};
        try {
     ...
     * @param command The command to be executed.
     */
    async testUnstoredInteraction(serialNumber, eventInteractionType, target, useHttps, useLocalCertificate, rejectUnauthorized, command, user, password) {
        let json = {};
        try {
            if (serialNumber !== "" && eventInteractionType >= 0 && eventInteractionType <= 12 && target !== "" && command !== "") {
            ...
            logging_1.rootAddonLogger.error(`Error occured at testUnstoredInteraction for device ${serialNumber}. code: ${e.code} | message: ${e.message.trim()}`);
            logging_1.rootAddonLogger.debug(`Error occured at testUnstoredInteraction for device ${serialNumber}.`, JSON.stringify(e));
            if (e.status) {
     ...
     * @param eventInteractionType The eventInteractionType.
     */
    async testStoredInteraction(serialNumber, eventInteractionType) {
        let json = {};
        try {
            ...
            logging_1.rootAddonLogger.error(`Error occured at testStoredInteraction for device ${serialNumber}. code: ${e.code} | message: ${e.message.trim()}`);
            logging_1.rootAddonLogger.debug(`Error occured at testStoredInteraction for device ${serialNumber}.`, JSON.stringify(e));
            if (e.status) {
                json = { "success": false, "reason": `Failed sent interaction command for eventInteractionType ${eventInteractionType} and device ${serialNumber}.`, "status": e.status };
            }
            else if (e.code) {
     ...
     */
    setupScheduledTasks() {
        logging_1.rootAddonLogger.info(`Setting up scheduled tasks...`);
        if (this.config.getStateUpdateIntervallActive()) {
            if (this.taskUpdateState) {
                logging_1.rootAddonLogger.info(`  getState already scheduled, remove scheduling...`);
            ...
            logging_1.rootAddonLogger.info(`  getState scheduled (runs every ${this.config.getStateUpdateIntervallTimespan()} minutes).`);
        }
        else {
            clearInterval(this.taskUpdateState);
            logging_1.rootAddonLogger.info(`  scheduling getState disabled in settings${this.config.getStateUpdateEventActive() === true ? " (state changes will be received by event)" : ""}.`);
        }
        logging_1.rootAddonLogger.info(`...done setting up scheduled tasks.`);
    }
    /**
    ...
    clearScheduledTasks() {
        if (this.taskUpdateState) {
            logging_1.rootAddonLogger.info(`Remove scheduling for getState.`);
            clearInterval(this.taskUpdateState);
        }
        this.config.close();
    }
    ...
    setupScheduledTask(task, name) {
        if (task) {
            logging_1.rootAddonLogger.info(`Remove scheduling for ${name}.`);
            switch (name) {
                case "getState":
                    clearInterval(this.taskUpdateState);
                    break;
    ...
    async processInvitations() {
        ...
        const invites = await this.httpService.getInvites().catch(err => {
            const error = (0, error_1.ensureError)(err);
            logging_1.rootAddonLogger.error("Error getting invites from cloud", { error: (0, utils_1.getError)(error) });
            return error;
        });
        if (Object.keys(invites).length > 0) {
            ...
            if (confirmInvites.length > 0) {
                const result = await this.httpService.confirmInvites(confirmInvites).catch(err => {
                    const error = (0, error_1.ensureError)(err);
                    logging_1.rootAddonLogger.error("Error in confirmation of invitations", { error: (0, utils_1.getError)(error), confirmInvites: confirmInvites });
                    return error;
                });
                if (result) {
                    logging_1.rootAddonLogger.info(`Accepted received invitations`, confirmInvites);
                    refreshCloud = true;
                }
            }
        }
        const houseInvites = await this.httpService.getHouseInviteList().catch(err => {
            const error = (0, error_1.ensureError)(err);
            logging_1.rootAddonLogger.error("Error getting house invites from cloud", { error: (0, utils_1.getError)(error) });
            return error;
        });
        if (Object.keys(houseInvites).length > 0) {
            for (const invite of Object.values(houseInvites)) {
                const result = await this.httpService.confirmHouseInvite(invite.house_id, invite.id).catch(err => {
                    const error = (0, error_1.ensureError)(err);
                    logging_1.rootAddonLogger.error("Error in confirmation of house invitations", { error: (0, utils_1.getError)(error) });
                    return error;
                });
                if (result) {
                    logging_1.rootAddonLogger.info(`Accepted received house invitation from ${invite.action_user_email}`, { invite: invite });
                    refreshCloud = true;
                }
            }
        }
