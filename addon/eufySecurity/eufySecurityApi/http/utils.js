...
const encodeSmartSafeData = function (command, payload) {
    const header = Buffer.from(device_1.SmartSafe.DATA_HEADER);
    const size = Buffer.allocUnsafe(2);
    size.writeInt16LE(payload.length + 9);
    const versionCode = Buffer.from([device_1.SmartSafe.VERSION_CODE]);
    ...
    const hash = (0, exports.generateHash)(data);
    return Buffer.concat([data, Buffer.from([hash])]);
};
exports.encodeSmartSafeData = encodeSmartSafeData;
const decodeSmartSafeData = function (deviceSN, data) {
    if (data.readInt8(0) !== device_1.SmartSafe.DATA_HEADER[0] && data.readInt8(1) !== device_1.SmartSafe.DATA_HEADER[1]) {
        //TODO: raise Exception invalid data header
    }
    if ((0, exports.generateHash)(data.slice(0, data.length - 1)) !== data.readInt8(data.length - 1)) {
    ...
    const result = {
        ...
        responseCode: -1,
        data: (0, utils_1.decryptPayloadData)(encData, Buffer.from(deviceSN), Buffer.from(device_1.SmartSafe.IV, "hex"))
    };
