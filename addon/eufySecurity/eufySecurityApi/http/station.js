...
class Station extends tiny_typed_emitter_1.TypedEmitter {
    ...
    pinVerified = false;
    constructor(eufySecurityApi, api, station, ipAddress, publicKey = "") {
        super();
        ...
        this.p2pSession = new session_1.P2PClientProtocol(this.eufySecurityApi.getP2PConnectionType(), this.rawStation, this.api, this.getLANIPAddress(), this.eufySecurityApi.getLocalStaticUdpPortForStation(this.rawStation.station_sn), this.lockPublicKey);
    ...
    static async getInstance(eufySecurityApi, api, stationData, ipAddress) {
        let publicKey;
    ...
    updateRawProperty(type, value, source) {
        const parsedValue = parameter_1.ParameterHelper.readValue(type, value, this.getSerial(), this.log);
    ...
    getDeviceTypeString() {
        switch (this.rawStation.device_type) {
            ...
            case types_1.DeviceType.INDOOR_PT_CAMERA_1080:
                return `indoorcamera`;
            case types_1.DeviceType.WALL_LIGHT_CAM:
            case types_1.DeviceType.WALL_LIGHT_CAM_81A0:
                return "walllightcam";
    ...
    onParameter(channel, param, value) {
        const params = {};
        const parsedValue = parameter_1.ParameterHelper.readValue(param, value, this.getSerial(), this.log);
        if (parsedValue !== undefined) {
            params[param] = {
                value: parsedValue,
                source: "p2p"
        ...
        cameraInfo.params.forEach(param => {
            if (param.dev_type === Station.CHANNEL || param.dev_type === Station.CHANNEL_INDOOR || this.isIntegratedDevice()) {
                ...
                if (this.isIntegratedDevice()) {
                    ...
                    const parsedValue = parameter_1.ParameterHelper.readValue(param.param_type, param.param_value, this.getSerial(), this.log);
                    if (parsedValue !== undefined) {
    ...
    async panAndTilt(device, direction, command = 1) {
        ...
        this.log.debug(`Station pan and tilt - sending command`, { stationSN: this.getSerial(), deviceSN: device.getSerial(), direction: types_2.PanTiltDirection[direction], command });
