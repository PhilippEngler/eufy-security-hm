...
class PushClientParser extends events_1.EventEmitter {
    ...
    handleFullMessage() {
        switch (this.state) {
            case fid_model_1.ProcessingState.MCS_VERSION_TAG_AND_SIZE:
                this.onGotVersion();
                break;
            case fid_model_1.ProcessingState.MCS_TAG_AND_SIZE:
                this.onGotMessageTag();
                break;
            case fid_model_1.ProcessingState.MCS_SIZE:
                this.onGotMessageSize();
                break;
            case fid_model_1.ProcessingState.MCS_PROTO_BYTES:
    ...
    onGotMessageSize() {
        ...
        if (incompleteSizePacket) {
            ...
            this.state = fid_model_1.ProcessingState.MCS_SIZE;
            this.waitForData();
            return;
        }
        ...
        if (this.messageSize > 0) {
            this.state = fid_model_1.ProcessingState.MCS_PROTO_BYTES;
            this.waitForData();
        }
        else {
            this.onGotMessageBytes();
    ...
    getNextMessage() {
        this.messageTag = 0;
        this.messageSize = 0;
        this.state = fid_model_1.ProcessingState.MCS_TAG_AND_SIZE;
        this.waitForData();
    }
    getMinBytesNeeded() {
        switch (this.state) {
    ...
    buildProtobufFromTag(messageTag) {
        switch (messageTag) {
            ...
            case fid_model_1.MessageTag.StreamErrorStanza:
