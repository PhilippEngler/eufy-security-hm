import { createServer as createServerHttp, IncomingMessage, ServerResponse } from "node:http";
...
import { GuardMode } from "../eufySecurityApi";
import { OpenCcuSystemvariableGeneric } from "../eufySecurityApi/utils/models";
import { EsOpenCcuAddOn } from "../esOpenCcuAddOn";

let api: EufySecurityApi;
let openCCUAddOn: EsOpenCcuAddOn;

export class ApiServer {
    private serverHttp = createServerHttp();
    private serverHttps = createServerHttps();

    constructor(esApi: EufySecurityApi, esOpenCCUAddOn: EsOpenCcuAddOn) {
    ...
    public stopServer(): void {
        rootAddonLogger.info("Stoping http server...");
        this.serverHttp.closeAllConnections();
        rootAddonLogger.info("...http server stopped");
        rootAddonLogger.info("Stoping https server...");
            ...
            if (url.length > 1) {
                switch (url[1]) {
                    ...
                    case "downloadConfig":
                        ...
                        fileName = `config_${osHostname}_${this.getDateTimeAsString(new Date())}.json`;
                        break;
                    default:
     ...
     */
    private getDateTimeAsString(dateTime: Date): string {
     ...
     */
    private replaceLastChars(input: string, char: string, numberOfChars: number): string {
