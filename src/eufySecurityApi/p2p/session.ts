...
export class P2PClientProtocol extends TypedEmitter<P2PClientProtocolEvents> {
    ...
    constructor(localAddress: string, localPort: number, connectionType: P2PConnectionType, p2pDid: string, dskKey: string, stationSerial: string, deviceSNs: DeviceSerial = {}, log: Logger) {
        super();
    ...
    private loockupByLocalAddress(host : string) : void
    {
        var address : Address = {host : host, port : 32108};
    ...
    private handleMsg(msg: Buffer, rinfo: RemoteInfo): void {
        ...
        } else if (hasHeader(msg, ResponseMessageType.LOCAL_LOOKUP_RESP)) {
            if (!this.connected) {
                this._clearLookupTimeout();
